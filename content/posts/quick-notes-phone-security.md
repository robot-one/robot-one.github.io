...
### Whatsapp
**Whatsapp** is "encrypted" - no one else can read your messages, hear your calls, etc. EXCEPT if you backup Whatsapp to Google Drive. Then Google can read your messages (**not saying they do**, but they can) [^1]. Check your Whatsapp settings to enable "encrypted backups" [^2] to make it more secure and make sure that Google can't read your messages from the backup. Also enable a Whatsapp 2-Step-Verification PIN [^3], so that if there's a hack, someone can't just log in and read your messages [^4].
...
**TikTok** is literally spyware that tracks your interests and data and sends it to China [^5] [^6] (search for "TikTok Spyware" and read some articles). **Skype** is unencrypted and not very good, but it's old, so still common to use. 
